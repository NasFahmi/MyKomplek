...
<aside class="w-[280px] bg-white overflow-y-auto text-black hidden md:flex flex-col">
    <div class="flex items-center justify-center gap-3 mb-12 mt-7">
        <img src="{{ asset('images/logo.png') }}" alt="" srcset="" class="w-10 h-auto">
    ...
    <nav class="flex flex-col pr-4 mb-12 gap-9">
        @foreach ($menus as $menu)
            <a href="{{ route($menu['route']) }}"
               ...
               {{ Str::startsWith($currentRoute, $menu['route']) ? 'text-[#6c8bf5] font-medium' : 'text-[#B1B1B1] hover:text-[#7a7a7a]' }}">
                @if (Str::startsWith($currentRoute, $menu['route']))
                    <span class="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-6 bg-[#6c8bf5] rounded-r-md"></span>
                @endif
                {{-- <p class="w-6 h-6 ml-3.5 font-bold">{{ $menu['icon'] }}</p> --}}
...
<div class="flex md:hidden">
    <div x-show="sidebarOpen" x-transition:enter="transition-opacity ease-linear duration-300"
        x-transition:enter-start="opacity-0" {{-- Baris enter-end dihapus agar opacity kembali ke state asli (bg-opacity-20) --}}
        x-transition:leave="transition-opacity ease-linear duration-300" x-transition:leave-end="opacity-0"
        @click="sidebarOpen = false" class="fixed inset-0 z-40 bg-black bg-opacity-20" style="opacity: 0.2" x-cloak>
    </div>
    ...
    <div x-show="sidebarOpen" x-transition:enter="transition ease-in-out duration-300 transform"
        ...
        {{-- Konten sidebar (tombol close, logo, menu) diletakkan di sini --}}
        <div class="flex items-center justify-between p-4 mt-2 mb-12">
            <button @click="sidebarOpen = false">
                <svg class="w-8 h-8 text-black" fill="currentColor" viewBox="0 0 24 24">
                    <path
                        d="M6.995 7.006a1 1 0 0 1 1.414 0L12 10.586l3.591-3.58a1 1 0 1 1 1.414 1.414L13.414 12l3.59 3.591a1 1 0 0 1-1.414 1.414L12 13.414l-3.591 3.591a1 1 0 0 1-1.414-1.414L10.586 12 6.995 8.409a1 1 0 0 1 0-1.403z" />
                </svg>
            </button>
            <div class="flex items-center justify-center w-7/12">
                <img src="{{ asset('images/logo.png') }}" alt="" srcset="" class="w-20 h-auto">
            </div>
        </div>
