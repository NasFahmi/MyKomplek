        ...
        <div class="p-6 bg-white border-l-4 border-yellow-500 rounded-lg shadow">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <h3 class="text-2xl font-bold text-gray-800">{{ $currentMonthPayments->count() }}/{{ $totalHouses }}</h3>
            ...
            <div class="mt-4">
                <div class="w-full bg-gray-200 rounded-full h-2.5">
                    @if ($totalHouses >0)
                    <div class="bg-yellow-500 h-2.5 rounded-full" style="width: {{ ($currentMonthPayments->count() / $totalHouses) * 100 }}%"></div>
        ...
        <div class="p-6 bg-white border-l-4 border-purple-500 rounded-lg shadow">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <h3 class="text-2xl font-bold {{ $balance >= 0 ? 'text-purple-600' : 'text-red-600' }}">Rp {{ number_format($balance, 0, ',', '.') }}</h3>
            ...
            <div class="flex mt-4 space-x-4">
                <div class="flex-1">
                    ...
                    <p class="text-sm font-semibold text-green-600">Rp {{ number_format($totalIncome, 0, ',', '.') }}</p>
                </div>
                <div class="flex-1">
                    <p class="text-xs text-gray-500">Pengeluaran</p>
                    <p class="text-sm font-semibold text-red-600">Rp {{ number_format($totalExpense, 0, ',', '.') }}</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 gap-6 mb-8 lg:grid-cols-2">
        ...
        <div class="overflow-hidden bg-white rounded-lg shadow">
                ...
                <div class="px-6 py-4 hover:bg-gray-50">
                    <div class="flex items-center justify-between">
                        ...
                        <div class="text-right">
                            <p class="font-semibold text-gray-900">Rp {{ number_format($fee->amount, 0, ',', '.') }}</p>
            ...
            <div class="px-6 py-3 text-right bg-gray-50">
                <a href="{{ route('pembayaran.index') }}" class="text-sm font-medium text-blue-600 hover:text-blue-500">Lihat Semua →</a>
            </div>
        </div>
            ...
            <div class="divide-y divide-gray-200">
                ...
                <div class="px-6 py-4 hover:bg-gray-50">
                    <div class="flex items-center justify-between">
                        ...
                        <div class="text-right">
                            <p class="font-semibold text-gray-900">Rp {{ number_format($payment->paymentDetail->sum('amount'), 0, ',', '.') }}</p>
    ...
    <div class="p-6 mb-8 bg-white rounded-lg shadow">
        <div class="flex flex-col items-start justify-between mb-6 sm:flex-row sm:items-center">
            ...
            <div class="mt-2 sm:mt-0">
                <select id="chartRange" class="block w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm">
    ...
    <div class="overflow-hidden bg-white rounded-lg shadow">
        <div class="px-6 py-4 border-b border-gray-200">
            <div class="flex items-center justify-between">
                ...
                <a href="{{ route('pengeluaran.create') }}" class="inline-flex items-center px-3 py-1 text-sm font-medium leading-5 text-white transition duration-150 ease-in-out bg-blue-600 border border-transparent rounded-md hover:bg-blue-500 focus:outline-none focus:border-blue-700 focus:shadow-outline-blue active:bg-blue-700">
                    Tambah Pengeluaran
                </a>
            </div>
        </div>
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 text-sm text-gray-500 whitespace-nowrap">
                            <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full 
                                {{ $expense->category === 'routine' ? 'bg-green-100 text-green-800' : 
                                   ($expense->category === 'emergency' ? 'bg-red-100 text-red-800' : 'bg-blue-100 text-blue-800') }}">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.getElementById('chartRange').addEventListener('change', function() {
            ...
            window.location.href = '{{ route("dashboard.index") }}?months=' + months;
        });
    });
</script>
@endsection